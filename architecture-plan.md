...
> 当前状态核实：2026-09-11。§0、§1.1、§9～§12 为当前执行依据；§1.2～§8 保留历史分析与决策，不能把其中“现状”直接当作今天的代码状态。§10～§12 更新了 §9 的后续事项。
...
| 级别 | 问题 | 影响 |
|------|------|------|
| P0 | 完整同步冲突协议与持久待发队列 | 已有拒写回执、双方备份和解决入口；后端支持可选 baseRevs，前端基线/持久 outbox 尚未接入，普通同步仍是兼容协议 |
| P1 | 多页面/多存储写入一致性 | 本轮已等待课程落盘；整库恢复尚非跨存储原子操作，同源多标签页仍可能写旧快照 |
| P1 | 移除大库隐式全量加载与全量渲染 | 已修首批误入全量分支、复习按批补详情、统计每页50条；索引仍整库驻留，极端过滤仍可能扫描全部分片 |
| P1 | 8000 句内容及性能验收 | 分片能力已具备，但不是已通过真实移动设备规模验收 |
| P2 | 提取页面服务边界、同步下行接入 | 避免继续向 main.html/core.js 堆逻辑；服务端增量下行尚未完整接入客户端 |
