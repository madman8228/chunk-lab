...
**尚未完成的第二阶段**
- 当前“进入某个内置题库”会加载该题库声明的全部分片；8000 句上线前应根据设备网络与练习批次增加内容索引/分批队列，避免低端移动设备一次解析整库。
- 内容分片目前依赖 Service Worker 的运行时缓存，下一步可增加 IndexedDB 内容缓存、下载进度和版本清理策略。
- 8000 句真正入库前仍需补齐内容质量流水线：稳定 cid、重复检测、chunk 拼接、干扰项覆盖率和人工抽检。

**验证**：内容构建成功；部署依赖检查通过；SW 专项 12/12；全量项目单测通过；浏览器回归 32/32。
