(() => {
  ...
  function normalizedAnsweredTotal(input) {
  ...
  function ymd(value) {
    const date = value instanceof Date ? value : /* @__PURE__ */ new Date();
    const pad = (number) => number < 10 ? "0" + number : "" + number;
    return date.getFullYear() + "-" + pad(date.getMonth() + 1) + "-" + pad(date.getDate());
  }
  function answerStatsAudit(mem) {
    const stats = mem && mem.stats || {}, by = stats.bySentence || {};
    const events = Array.isArray(stats.events) ? stats.events : [], seen = /* @__PURE__ */ new Set();
  ...
  function dailyActivity(mem) {
    const stats = mem && mem.stats || {}, days = {}, seen = /* @__PURE__ */ new Set();
    ...
    Object.keys(rounds).forEach((key) => {
      days[key].rounds = Math.max(days[key].rounds, rounds[key]);
    });
    return days;
  }
  function streakDays(mem, now) {
    const log = dailyActivity(mem);
    const date = now instanceof Date ? new Date(now.getTime()) : /* @__PURE__ */ new Date();
    let count = 0;
  ...
  function bumpDaysLog(mem, now) {
    ...
    const key = ymd(now instanceof Date ? now : /* @__PURE__ */ new Date());
  ...
  function backfillDaysLog(mem) {
    ...
    Object.keys(counts).forEach((key) => {
      mem.stats.daysLog[key] = { rounds: counts[key] };
    });
    return Object.keys(counts).length;
  }
  var CoreActivity = Object.freeze({ normalizedAnsweredTotal, ymd, answerStatsAudit, dailyActivity, streakDays, todayRounds, bumpDaysLog, backfillDaysLog });

  // scripts/core-activity-entry.mjs
  if (typeof globalThis !== "undefined") globalThis.CoreActivity = CoreActivity;
})();
