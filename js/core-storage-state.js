(() => {
  ...
  function buildStatsPersistencePlan(stats, previousSignatures, previousEventSnapshot, forceFullRewrite, statSignature, eventSnapshot) {
    ...
    if (forceFullRewrite) {
      Object.keys(by).forEach((key) => {
        nextSignatures[key] = signatureOf(by[key]);
      });
      return {
        by,
        events,
        dirty: Object.keys(by),
        gone: [],
        changed: by,
        eventRows: events,
        eventFull: true,
        replaceStats: true,
        nextSignatures,
        nextEventSnapshot: snapshotOf(events)
      };
    }
    Object.keys(by).forEach((key) => {
    ...
    const prior = previousEventSnapshot && typeof previousEventSnapshot === "object" ? previousEventSnapshot : null;
    let canAppend = false;
    if (prior && events.length >= prior.count) {
      if (prior.count === 0) canAppend = true;
      else canAppend = !!(events[0] && events[0].id) && events[0].id === prior.firstId && !!(events[prior.count - 1] && events[prior.count - 1].id) && events[prior.count - 1].id === prior.lastId;
    }
  ...
  function buildCourseProgressWritePlan(kind, snapshot, diskBase, current, mergeCoursesList, mergeKeyedMap) {
    ...
    const nextSnapshot = kind === "courses" ? mergeCourses(diskBase, snapshot, current) : mergeMap(diskBase || {}, snapshot && typeof snapshot === "object" ? snapshot : {}, current || {});
  ...
  var CoreStorageState = Object.freeze({
    CURRENT_VERSION,
    MIGRATIONS,
    migrate,
    defaultMem,
    mergeCourseProgressSources,
    buildBusinessProjection,
    buildStatsBusinessMeta,
    buildStatsPersistencePlan,
    buildCourseProgressWritePlan,
    parseLegacyStatsRaw
  });

  // scripts/core-storage-state-entry.mjs
  if (typeof globalThis !== "undefined") globalThis.CoreStorageState = CoreStorageState;
})();
