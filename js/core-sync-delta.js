(() => {
  ...
  var CoreStatsSignature = Object.freeze({ statSig, statsSig, eventSnapshot });

  // src/core/sync-delta.mjs
  ...
  function buildStatsDelta(mem, marks) {
    ...
    const statSig2 = CoreStatsSignature.statSig;
    const sbs = {};
    const sbsGone = [];
    ...
    const evs = previousEvents === null || previousEvents === void 0 ? events : events.filter((event) => event && event.id && !previousEvents[event.id]);
    const nextBs = {};
    if (previousStats) Object.keys(previousStats).forEach((key) => {
      nextBs[key] = previousStats[key];
    });
    Object.keys(bySentence).forEach((key) => {
      nextBs[key] = statSig2(bySentence[key]);
    });
    sbsGone.forEach((key) => {
      delete nextBs[key];
    });
    const nextEvIds = {};
    if (previousEvents) Object.keys(previousEvents).forEach((key) => {
      nextEvIds[key] = 1;
    });
    events.forEach((event) => {
      if (event && event.id) nextEvIds[event.id] = 1;
    });
  ...
  function ackPending(pending, sentUp, sentGone) {
    Object.keys(sentUp || {}).forEach((id) => {
      delete pending.dirty[id];
    });
    Object.keys(sentGone || {}).forEach((id) => {
      delete pending.gone[id];
    });
    pending.all = false;
  }
  ...
  var CoreSyncDelta = Object.freeze({
    memForCloud,
    buildStatsDelta,
    sameValue,
    createPendingSet,
  ...
  // scripts/core-sync-delta-entry.mjs
  if (typeof globalThis !== "undefined") globalThis.CoreSyncDelta = CoreSyncDelta;
})();
