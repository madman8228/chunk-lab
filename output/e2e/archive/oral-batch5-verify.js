/**
 * oral-batch5-verify.js · 第五批口语（119 句）入库后的端到端验收
 *
 ...
 *
 * 检查项：
 *   A. 题库列表页里 builtin-daily 的题数（346 → 465）
 *   B. ensureDeck 全量审计：条目数、抽样新句在库、结构完整性
 *   C. 练习区真机截图（?direct=1）
 *   B6. 负向自证：同一份判据注入坏样本必须判红
 *
 * 运行：node output/e2e/oral-batch5-verify.js
...
const OUT = path.join(__dirname, 'oral-batch5');
fs.mkdirSync(OUT, { recursive: true });
const PORT = require(path.join(ROOT, 'e2e', 'lib', 'free-port')).freePort(8930, 30);
const TMP_DB = fs.mkdtempSync(path.join(os.tmpdir(), 'cl-oral5-'));
const BASE = 'http://127.0.0.1:' + PORT;
const EXPECT_TOTAL = 465;   /* 88（builtins.js）+ 377（oral8000.js） */
let server = null;
...
/* 本批抽样：6 个子批各取 1~2 条，必须在库。
   注意：Ma'am 那条带逗号，字符串要与 oral8000.js 里的 sentence 完全一致。 */
const SAMPLES = [
  'Go clean up.',           /* 批1 洗漱 */
  'Rinse it.',              /* 批1 洗漱 */
  'It\'s getting windy.',   /* 批2 天气 */
  'Take some medicine.',    /* 批3 身体就医 */
  'Ma\'am, you called us.', /* 批4 电话 */
  'Sleep in.',              /* 批5 睡眠 */
  'Give me a thumbs up.'    /* 批6 家务居家 */
];
...
(async function main() {
  ...
  try {
    ...
    check('A1 builtin-daily 题数 346 → ' + EXPECT_TOTAL, dailyCount === EXPECT_TOTAL, '实际 ' + dailyCount);
