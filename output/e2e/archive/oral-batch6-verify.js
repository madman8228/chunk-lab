/**
 * oral-batch6-verify.js · 第六批口语（31 句）入库后的端到端验收
 *
 * 目的：不靠读源码，靠真实浏览器确认「新句子确实进了应用、且能正常出题」。
 *
 * ⚠️ 关键（第四批踩过的坑，这里沿用正确入口）：内置题库的条目**不在** window.BUILTIN 里
 *    —— 页面只加载 builtins.js 的基础 88 句，其余按需从 content/ 分片取
 *    （js/content-repository.js）。审计必须走 window.ContentRepo.ensureDeck(id, mem)
 *    （= 练习页实际用的入口）；用 CL.builtinDecks() 只会看到 88 条 → 假红。
 *
 * 检查项：
 *   A. 题库列表页里 builtin-daily 的题数（465 → 496）
 *   B. ensureDeck 全量审计：条目数、抽样新句在库、结构完整性
 *   C. 练习区真机截图（?direct=1）
 *   B6. 负向自证：同一份判据注入坏样本必须判红
 *
 * 运行：node output/e2e/oral-batch6-verify.js
...
const OUT = path.join(__dirname, 'oral-batch6');
fs.mkdirSync(OUT, { recursive: true });
const PORT = require(path.join(ROOT, 'e2e', 'lib', 'free-port')).freePort(8940, 30);
const TMP_DB = fs.mkdtempSync(path.join(os.tmpdir(), 'cl-oral6-'));
const BASE = 'http://127.0.0.1:' + PORT;
const EXPECT_TOTAL = 496;   /* 88（builtins.js）+ 408（oral8000.js） */
let server = null;
...
/* 本批抽样：3 个子批各取 2~3 条，必须在库。
   注意：字符串要与 oral8000.js 里的 sentence 完全一致（含标点）。 */
const SAMPLES = [
  'I want to stay in bed for another half an hour.',  /* 批1 起床 */
  'It\'s still early.',                              /* 批1 起床 */
  'Why didn\'t the alarm go off?',                   /* 批1 起床 */
  'I feel dizzy.',                                   /* 批2 身体 */
  'I have trouble waking up in the morning.',        /* 批2 睡眠 */
  'Fold up your bedding.',                           /* 批2 家务 */
  'I thought she said something to me.',             /* 批3 对话 */
  'Yeah, maybe run into her at the gym.'             /* 批3 对话 */
];
...
(async function main() {
  ...
  try {
    ...
    check('A1 builtin-daily 题数 465 → ' + EXPECT_TOTAL, dailyCount === EXPECT_TOTAL, '实际 ' + dailyCount);
      ...
      return ContentRepo.ensureDeck('builtin-daily', m).then(function (deck) {
        ...
        window.__oralShapeOk = function (it) {
          ...
          /* ⚠️ explanations 的长度**不是**与 chunks 对齐的（它是"讲解条目"列表，长度自由）。
             第四批在这里写成 `explanations.length === n` → 79 条假红。契约只要求"有讲解"。 */
    ...
    console.log('  （范围：builtin-daily。「每槽 ≥2」是该库的不变量 —— builtins.js 的 You said it. 已在第四批补齐；');
    console.log('    freq-idioms.js 仍有 4 处单条槽，属另一题库、非本批范围，项目闸只拦空槽。）');
