...
function rowDelta(map, mark, rowSig) {
  ...
  const gone = [];
  if (mark === null) {
    Object.keys(source).forEach((key) => { up[key] = source[key]; });
  } else {
  ...
  const next = {};
  if (mark) Object.keys(mark).forEach((key) => { next[key] = mark[key]; });
  Object.keys(up).forEach((key) => { next[key] = rowSig(source[key]); });
...
function buildEntityDelta(mem, marks) {
  ...
  const reinforceMap = {};
  reinforceBook.forEach((item) => { if (item && item._key) reinforceMap[item._key] = item; });
  const current = marks || {};
  return {
...
function alignEntityMarks(mem) {
  ...
  Object.keys(mastered).forEach((key) => { out.mastered[key] = sigMasteredRow(mastered[key]); });
  Object.keys(deletedItems).forEach((key) => { out.deletedItems[key] = sigDeletedRow(); });
  reinforceBook.forEach((item) => { if (item && item._key) out.reinforceBook[item._key] = sigReinforceRow(item); });
  return out;
}

export const CoreEntityDelta = Object.freeze({
  strHash,
  sigMasteredRow,
  sigReinforceRow,
  sigDeletedRow,
