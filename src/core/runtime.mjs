/*
 * Small runtime coordination primitives shared by browser-facing modules.
 *
 * This module deliberately knows nothing about storage, accounts, or sync
 * protocol details.  It only owns the lifecycle of a single in-flight task so
 * callers can share one Promise without duplicating startup work.
 */
...
export function createWriteCoordinator(options = {}) {
...
export function createLatestWriteLane(options = {}) {
  ...
  function start(entry) {
    active = entry;
    Promise.resolve()
      .then(() => options.start(entry))
      .then((value) => finish(entry, value), (error) => finish(entry, null, error));
  }
  ...
  return Object.freeze({
    enqueue(entry) {
      ...
      const error = typeof options.onOverflow === 'function'
        ? options.onOverflow(entry)
        : new Error('write queue is full');
      entry.reject(error);
      return entry.promise;
    },
...
export function createTabNotifier(options = {}) {
  ...
  function publish(area) {
    ...
    try {
      ...
      /* The BroadcastChannel path remains usable when the beacon storage is unavailable. */
    }
    return message;
  }
  ...
  function receiveStorageEvent(event, decodeKey) {
    ...
    if (key === syncKey) {
      ...
      try { return receive(JSON.parse(event.newValue)); } catch (_) { return null; }
    }
    if (key === storeKey) {
      externalSequence += 1;
