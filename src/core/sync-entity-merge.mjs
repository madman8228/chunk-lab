...
function copyRevisions(revisions) {
  ...
  Object.keys(source).forEach((group) => {
    result[group] = source[group] && typeof source[group] === 'object'
      ? { ...source[group] } : {};
  });
  return result;
}
...
function mergeEntityList(localList, remoteList, localRevisions, remoteRevisions, group, idOf) {
  ...
  remote.forEach((item) => {
    const id = item && idOf(item);
    if (!id) return;
    if (!seen[id]) { seen[id] = 1; keys.push(id); }
    right[id] = item;
  });
...
function mergeEntityMap(localMap, remoteMap, localRevisions, remoteRevisions, group) {
  ...
  Object.keys((remoteRevisions && remoteRevisions[group]) || {}).forEach((id) => {
    if (!Object.prototype.hasOwnProperty.call(remote, id)
      && safeRev(remoteRevisions, group, id) > safeRev(localRevisions, group, id)) {
      delete value[id];
...
export const CoreSyncEntities = Object.freeze({ mergeEntityList, mergeEntityMap });
