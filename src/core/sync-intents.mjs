...
function buildReceipts(options) {
  ...
  const progressPayload = input.progressPayload && typeof input.progressPayload === 'object'
    ? input.progressPayload
    : {};
  const receipts = [];
  ...
  (Array.isArray(input.intents) ? input.intents : []).forEach((record) => {
    ...
    const value = record.entity === 'courses'
      ? findCourseValue(coursePayload, record.id)
      : progressPayload[record.id];
    if (value !== undefined && equalJson(value, record.value)) receipts.push(receipt(record));
  });
  ...
  (Array.isArray(input.learningIntents) ? input.learningIntents : []).forEach((record) => {
    if (!record) return;
    const matches = record.entity === 'events'
      ? !record.deleted && !!sentEvents[record.id]
      : record.deleted
        ? sentGone.has(record.id)
        : own(statsDelta.sbs, record.id) && sameValue(statsDelta.sbs[record.id], record.value);
...
export const CoreSyncIntents = Object.freeze({ buildReceipts });
