...
function buildSyncPayload(options) {
  ...
  const protectedCourses = input.protectedCourses || Object.create(null);
  const protectedProgress = input.protectedProgress || Object.create(null);
  ...
  if (entityDelta && (deltaHasRows(entityDelta.mastered)
    || deltaHasRows(entityDelta.reinforceBook)
    || deltaHasRows(entityDelta.deletedItems))) {
    payload.entityDelta = {};
  ...
  const sentEvents = Object.create(null);
...
export const CoreSyncPayload = Object.freeze({ buildSyncPayload });
