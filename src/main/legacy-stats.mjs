/* Pure migration for statistics written against temporary deck ids. */
...
export function migrateLegacyStatKeys(bySentence, decks, options = {}) {
  if (!bySentence || typeof bySentence !== 'object') return { changed: false, by: bySentence };
  const list = Array.isArray(decks) ? decks : [];
  const normalize = typeof options.normalizeSentence === 'function'
    ? options.normalizeSentence
